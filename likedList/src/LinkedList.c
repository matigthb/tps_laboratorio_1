...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    if(this != NULL && nodeIndex > -1 && nodeIndex <= ll_len(this))
    {
    	pNode = (Node*) malloc(sizeof(Node)*1);
    	...
    	if(pNode != NULL)
    	{
    		...
    		if(nodeIndex != 0)
    		{
    			pNodeLast = getNode(this, nodeIndex - 1);

    			if(pNodeLast != NULL)
    			{
    				pNodeLast->pNextNode = pNode;
    				returnAux = 0;
    			}
    		}
    		else
...
int ll_remove(LinkedList* this,int index)
{
	...
	if(this!=NULL && index > -1 && index < ll_len(this))
	{
		...
		if(pNodo != NULL)
		{
			...
			{
				pNodoAux = getNode(this, index - 1);

				if(pNodoAux != NULL)
				{
					pNodoAux->pNextNode = pNodo->pNextNode;
				}
			}
	...
	if(this != NULL && order >= 0 && order <= 1 && pFunc != NULL)
	{
		for(int i = 0; i < ll_len(this) - 1; i++)
		{
			for(int j = i + 1; j < ll_len(this); j++)
			{
				...
				if(pFunc(pElement1, pElement2) > 0 && order == 1) //Si el primer elemento es menor al segundo elemento, orden ascendente.
				{
				...
				{
					if(pFunc(pElement1, pElement2) <= 0 && order == 0) //Si el primer elemento es menor al segundo elemento, orden descendente.
					{
						ll_set(this, i, pElement2);
						ll_set(this, j, pElement1);
						returnAux = 0;
